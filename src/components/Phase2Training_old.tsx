...
export const Phase2Training = ({ onComplete }: Phase2TrainingProps) => {
  ...
  const handleTimerExpired = () => {
    setFeedbackMessage('⏱️ Time expired! Stay calm and analyze faster next time.');
    setIsCorrect(false);
  ...
  const handleStep1 = (tactic: TacticOption) => {
    ...
    if (correct) {
      setFeedbackMessage('✓ Correct! Tactic identified.');
      setIsCorrect(true);
      setShowFeedback(true);
      setTimeout(() => {
      ...
      setFeedbackMessage('✗ Not quite. Focus on the core manipulation.');
      setIsCorrect(false);
      setShowFeedback(true);
      setTimerPaused(true);
      setTimeout(() => {
  ...
  const handleStep2 = (verification: VerificationOption) => {
    ...
    setFeedbackMessage('✓ Good verification strategy!');
    setIsCorrect(true);
    setShowFeedback(true);
    setTimeout(() => {
      setShowFeedback(false);
  ...
  const handleStep3 = (action: ActionOption) => {
    ...
    if (successCount >= 3) {
      setFeedbackMessage('🎯 Pattern mastered! You resisted this manipulation.');
      setIsCorrect(true);
      setShowFeedback(true);
      setTimeout(() => {
        setIsComplete(true);
      ...
      setFeedbackMessage(`✓ Scam defused! Streak: ${successCount}/3`);
      setIsCorrect(true);
      setShowFeedback(true);
      setTimeout(() => {
        nextRound();
      ...
      <div className="relative z-10 min-h-screen flex flex-col items-center justify-center px-4 py-12">
        ...
        >
          ...
          <p className="text-gray-300 text-lg">
            Cyber fire drill • Adversarial AI pressure training
          </p>
        </motion.div>
